...
public class WalletTransferActivity extends ParentActivity{
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.wallettransfer);
		setlayoutRef();
	}
	...
	private void goServer() {
		Map<String,String> requset=RequsetManager.getIntance().balanceTransferRequset(Utils.getValueFromSharedPreferences(this,SPKeyConstants.SHARED_PREF_USERNAME), 
				enterredNumber, enterredAmount);	
		WebServiceCallerAsyncTask servcie=new WebServiceCallerAsyncTask(this);
		servcie.startService(ServiceConfiguration.LOgin_Url,requset, ServiceConfiguration.BalanceTransaction_METHOD);
	}
	public void sendResponseBalanceTransaction(Object response){
		...
		if(response instanceof String){
			...
			String status=new ReponseParser().balanceTransResponse((SoapObject)response);
